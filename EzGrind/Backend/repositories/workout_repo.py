...
#
# THE DIVISOR MUST STAY 30.0, NEVER 30. reps is SMALLINT, so `reps / 30` is
# integer division in Postgres and floors to 0 for every set under 30 reps -
# which silently stores estimated_1rm = best_weight for the entire table and
# re-ranks every personal record wrong. It raises no error and looks plausible.
#
# 30.0 is a NUMERIC literal, not a float one, which also keeps the expression
# NUMERIC so ROUND(x, 2) resolves. There is no round(double precision, integer)
# in Postgres, so letting it drift to float is a hard error rather than a
# silent one - the reason a bound weight is cast with ::numeric below.
_EPLEY = "ROUND(%s::numeric * (1 + %s / 30.0), 2)"
_EPLEY_COLUMNS = "ROUND(ws.weight * (1 + ws.reps / 30.0), 2)"
_EPLEY_ORDER = "ws.weight * (1 + ws.reps / 30.0)"


def _upsert_personal_record(cursor, user_id, exercise_id, weight, reps, today):
    ...
    The MySQL version of this carried a warning that ASSIGNMENT ORDER WAS
    LOAD-BEARING: it compared each column against estimated_1rm, so writing
    estimated_1rm first made every later comparison test the new value against
    itself and left a row whose 1RM disagreed with its own weight and reps.

    That cannot be expressed here. Postgres evaluates every SET expression
    against the pre-update row, and the WHERE decides the whole row at once -
    so the record either moves to the better effort or is left alone, and
    there is no ordering to get wrong.
    """
    cursor.execute(f"""
        INSERT INTO personal_records
            (user_id, exercise_id, best_weight, best_reps, estimated_1rm, achieved_on)
        VALUES (%s, %s, %s, %s, {_EPLEY}, %s)
    ...
    """, (user_id, exercise_id, weight, reps, weight, reps, today))


def log_set(user_id, exercise_id, weight, reps, comments):
    """Find or create today's workout, then append one set to it.

    The upsert depends on UNIQUE (user_id, workout_date). Without the
    constraint every call inserts a fresh workout row instead of reusing the
    day's.
    ...
    closes it: the database decides, not the application.

    DO UPDATE with a no-op assignment, not DO NOTHING. DO NOTHING returns NO
    ROW on the conflict branch, so RETURNING would come back empty on exactly
    the path that matters - the second set of the day. Writing workout_date
    back to itself is what keeps the row in the result. The cost is that the
    identity sequence advances on every call including the conflicts, so
    workout_id has gaps. Gaps in a surrogate key mean nothing.

    "Today" is the APPLICATION's date, passed in rather than taken from the
    database with CURRENT_DATE. stats_repo decides streaks and week activity
    with Python's date.today(); once the database is not the same machine as
    the web process - which is the whole point of moving to Neon, running UTC -
    those two clocks disagree, and a session logged late in the evening files
    under tomorrow and breaks the streak it should have extended. One clock.
    ...
    with get_cursor(dictionary=False) as cursor:
        ...
        # removed from the form, but the column and its history remain.
        cursor.execute("""
...
def delete_set(user_id, set_id):
    ...
    with get_cursor(dictionary=False) as cursor:
        ...
        # DELETE ... USING, which is Postgres' spelling of MySQL's
        # multi-table DELETE. The joined table goes in USING and only the
        # table named after DELETE FROM loses rows.
        cursor.execute("""
            DELETE FROM workout_sets ws
            USING workouts w
            WHERE w.workout_id = ws.workout_id
              AND ws.set_id = %s AND w.user_id = %s
        """, (set_id, user_id))
...
def exercise_stats(user_id, exercise_id, recent_limit=10):
    ...
    recent list answer different shapes of question, and folding them into one
    statement would mean either a cross join or three CTEs stitched by a
    dummy key - more SQL to read for no round trips saved that matter. All
    three are constant-cost for one user and one exercise.
    """
    with get_cursor() as cursor:
        cursor.execute("""
        ...
        cursor.execute(f"""
            SELECT ws.weight, ws.reps, w.workout_date,
                   {_EPLEY_COLUMNS} AS estimated_1rm
            FROM workout_sets ws
            JOIN workouts w ON w.workout_id = ws.workout_id
            WHERE w.user_id = %s AND ws.exercise_id = %s
              AND ws.weight IS NOT NULL AND ws.reps IS NOT NULL
...
def sessions(user_id, page=1, limit=20, date_from=None, date_to=None,
    ...
    bounded set of workouts. Postgres would also accept IN (... LIMIT ...),
    which MySQL rejects outright, but the derived table is kept: it makes the
    "pick the page, then fetch its sets" shape visible in the query rather
    than leaving it to be inferred from a subquery's plan.
