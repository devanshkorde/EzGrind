-- =====================================================================
-- muscle_groups_and_exercises.sql
-- =====================================================================
-- Reference data. Without this, a freshly created database has empty muscle
-- and exercise dropdowns, so no workout can be logged and every downstream
-- feature is dead.
--
-- WHY EXPLICIT IDs
-- INSERT IGNORE only skips a row when it collides with a PRIMARY KEY or a
-- UNIQUE key. Writing the ids out means the collision happens on the primary
-- key, so this file is genuinely re-runnable even on a database that has not
-- yet had migration 002's UNIQUE(muscle_name) applied. The ids below match
-- the rows already present in the reference database.
--
-- Existing rows are never modified - INSERT IGNORE skips them, so any
-- descriptions you have edited by hand survive re-running this file.
--
-- Requires: 001_baseline.sql
-- Applied by: mysql -u root -p ezgrind_db < muscle_groups_and_exercises.sql
-- =====================================================================
...
-- ---------------------------------------------------------------------
-- Muscle groups - one per PNG in Frontend/assets/muscles/
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
INSERT IGNORE INTO exercises (exercise_id, exercise_name, muscle_id, equipment, description) VALUES
    -- Chest
