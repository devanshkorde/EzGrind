...
def _now():
    """The current instant, with its zone attached.

    expires_at and used_at are TIMESTAMPTZ. Binding a NAIVE datetime to one
    makes Postgres interpret it in the session's time zone, which on Neon is
    UTC and on this laptop is not - so a token minted with a naive
    datetime.now() + 20 minutes would be born already expired, or valid for
    hours, depending on which side of UTC you live. Comparing a naive value
    against a column written by the server's now() has the same problem in
    reverse.

    One aware clock on both sides removes the question.
    """
...
def _lock_user(cursor, user_id):
    ...
    deadlock-free. Under MySQL that was not theoretical: "UPDATE ... WHERE
    user_id = X" followed by an INSERT took row AND GAP locks in an order that
    depended on what was already in the table, so two resets requested for one
    account in the same instant could each hold what the other needed. It
    fired as error 1213 the first time it was tried.

    Postgres has no gap locks, so that specific deadlock cannot happen here.
    This is kept anyway, for the property it buys rather than the bug it fixed:
    it serialises concurrent resets for one account, so the "exactly one live
    token survives" invariant is enforced by the lock rather than by hoping two
    UPDATEs interleave favourably. One row, always the same row, always first.
    """
