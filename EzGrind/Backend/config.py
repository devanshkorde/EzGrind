...
def _with_sslmode(url):
    """Guarantee sslmode=require on the connection string.

    Neon refuses any connection that is not TLS, but libpq's default
    sslmode=prefer DOWNGRADES silently rather than failing - so a URL pasted
    without the parameter works against a plaintext local Postgres and then
    quietly decides encryption is optional everywhere else. Forcing it here
    means the guarantee does not depend on anyone remembering to type it.
    """
    parts = urlparse(url)
...
# One string, not five variables. Neon issues exactly one connection string,
# and splitting it into host/port/user/password/database means five chances to
# transcribe one of them wrong for no gain.
_RAW_DATABASE_URL = _required("DATABASE_URL")
if not _RAW_DATABASE_URL.startswith(("postgresql://", "postgres://")):
    # The value is deliberately not echoed - it carries the password.
    raise ConfigError(
        "DATABASE_URL must be a PostgreSQL connection string starting with "
        "postgresql://\n"
        "  postgresql://user:password@host/dbname?sslmode=require\n"
        "  Copy it from the Neon dashboard: Connection Details -> psycopg2."
    )
DATABASE_URL = _with_sslmode(_RAW_DATABASE_URL)
...
# The ceiling is Neon's, not the driver's: a free-tier project allows a limited
# number of direct connections, and a pool that tries to exceed it fails at
# connect time rather than at review time. Rejecting a bad value here beats a
# PoolError surfacing on the first request that touches the database.
DB_POOL_SIZE = _int("DB_POOL_SIZE", "5")
if not 1 <= DB_POOL_SIZE <= 20:
    raise ConfigError(f"DB_POOL_SIZE must be between 1 and 20, got {DB_POOL_SIZE}")
