...
if resp.status_code == 503:
    ...
    sys.exit("\nDatabase unreachable - check DATABASE_URL in .env and re-run.")
...
if WRITE:
    ...
    account = {
        # Must satisfy validators.validate_password, which runs on every write
        # of a password. "smoke-password" predates that policy and fails three
        # rules at once - no uppercase, no digit, and it contains "smoke",
        # which is a fragment of both full_name and the email local part.
        "full_name": "Smoke Test", "email": email, "password": "Gr1nd-Verify!9",
    ...
    # The stored 1RM must agree with its own weight and reps.
    #
    # This is the assertion that catches Postgres integer division. reps is
    # SMALLINT, so an Epley written `reps / 30` instead of `reps / 30.0` floors
    # to 0 for every set under 30 reps and stores estimated_1rm = best_weight -
    # here, 65.0 where 75.83 is correct. It raises no error and every column
    # still looks plausible on its own; only the relationship between them is
    # wrong, which is exactly what this compares.
    ...
    # --- password change ------------------------------------------------
    # new_password has to be POLICY-VALID for this case to test what it claims.
    # change_password validates the replacement before it checks the current
    # password, so an invalid one fails as weak_password/new_password and never
    # reaches the current-password check this case exists to exercise.
    resp = show("password (wrong current)", user.post("/api/me/password", json={
        "current_password": "not-my-password", "new_password": "Unrelated-Pass!2"}))
