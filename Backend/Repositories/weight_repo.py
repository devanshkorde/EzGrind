...
def log_weight(user_id, weight_kg, logged_on=None):
    ...
    with get_cursor() as cursor:
        cursor.execute("""
            ...
            VALUES (%s, COALESCE(%s, CURDATE()), %s) AS new
            ON DUPLICATE KEY UPDATE weight_kg = new.weight_kg
        """, (user_id, logged_on, weight_kg))

        _resync_profile_weight(cursor, user_id)

        # Read the row back rather than trusting lastrowid, which is not
        # meaningful on the duplicate-key branch.
        cursor.execute("""
            SELECT log_id, logged_on, weight_kg
            FROM weight_logs
            WHERE user_id = %s AND logged_on = COALESCE(%s, CURDATE())
        """, (user_id, logged_on))
        row = cursor.fetchone()
