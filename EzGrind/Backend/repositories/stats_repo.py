...
def _current_streak(days):
    ...
    "Today" is the WEB PROCESS's date, and so is the date workout_repo.log_set
    stamps onto a workout - it binds date.today() rather than asking Postgres
    for CURRENT_DATE. That matters now the database is not the same machine:
    Neon runs UTC, and a session logged at 02:00 IST would be filed by the
    server under the previous day and break the streak it should have extended.
    One clock decides, and it is this one.

    KNOWN LIMITATION, unchanged: that clock is the SERVER's, not the user's.
    Someone training from a timezone behind the deployment still has a late
    session filed under the following day. Invisible with one user; real the
    moment there are two, and the fix then is a per-user timezone column rather
    than another wall clock.
    """
    trained = set(days)
    cursor = date.today()
...
def volume_series(user_id, period):
    ...
    date_trunc collapses the monthly case into the daily one: both branches now
    select a single date column, group by it and return it, so there is one
    shape to read instead of two and no reassembling a date out of a year and a
    month in Python.

    MySQL needed that reassembly. DATE_FORMAT was the obvious tool there, but
    its format string is made of literal % characters, which collide with the
    driver's own %s placeholders - escaped as %% they survive into MySQL, which
    reads %%Y as the literal text "%Y" and returns the format string instead of
    a date. That whole problem is gone.
    """
    cutoff = date.today() - timedelta(days=PERIOD_DAYS[period] - 1)

    # Whitelisted by construction: `period` reached here through choice_arg
    # against stats_repo.PERIOD_DAYS, so this is one of exactly two literals
    # and never anything a caller typed.
    bucket = ("date_trunc('month', w.workout_date)::date"
              if period == "year" else "w.workout_date")
