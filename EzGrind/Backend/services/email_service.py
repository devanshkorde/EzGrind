...
# The sender an unverified Resend account is restricted to. Sending from it
# works, which is what makes it dangerous: the account owner receives their test
# email, concludes email is fine, and every other recipient is 403'd.
...
def warn_if_unconfigured():
    ...
    elif config.EMAIL_BACKEND == "resend" and \
            config.MAIL_FROM_ADDRESS.lower() == SANDBOX_SENDER:
        # Cheap and certain: a config comparison, not an API call. The live
        # symptom is a per-send 403 buried in the log, which nobody reads until
        # a user reports never getting a reset link - so it is said out loud at
        # boot instead.
        logger.warning(
            "EMAIL_BACKEND=resend with MAIL_FROM_ADDRESS=%s, which is Resend's "
            "SANDBOX SENDER. It delivers only to the address that owns the "
            "Resend account and rejects every other recipient - so signups and "
            "password resets appear to succeed while nobody receives anything. "
            "Verify a domain at https://resend.com/domains and set "
            "MAIL_FROM_ADDRESS to an address on it, or use EMAIL_BACKEND="
            "console to print emails instead.",
