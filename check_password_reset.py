...
def require_schema():
    ...
    # Microsecond precision is load-bearing, not cosmetic: at whole-second
    # resolution a session opened in the same second as a reset compares as
    # newer than the reset and survives it. Checked here so the suite refuses
    # to run rather than passing case 6 by accident on a fast machine.
    with get_cursor() as cursor:
        cursor.execute("""
            SELECT COALESCE(DATETIME_PRECISION, 0) AS precision_digits
            FROM information_schema.COLUMNS
            WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'users'
              AND COLUMN_NAME = 'password_changed_at'
        """)
        row = cursor.fetchone()
        precision = row["precision_digits"] if row else 0

    if has_table and "password_changed_at" in columns and precision != 6:
        sys.exit(
            "users.password_changed_at is DATETIME with no fractional seconds.\n"
            "  At that resolution a session opened in the same second as a reset\n"
            "  is not invalidated. Apply migration 008:\n"
    ...
    if not has_table or "password_changed_at" not in columns:
        sys.exit(
            ...
            "  Apply it first (mysql.exe is not on PATH here, so use the full\n"
            "  path to the 8.0 client):\n"
            "    Get-Content ..\\Database\\migrations\\007_password_reset.sql | "
            '& "C:\\Program Files\\MySQL\\MySQL Server 8.0\\bin\\mysql.exe" '
            "-u root -p ezgrind_db"
        )
...
def check_concurrent_issue(user_id):
    ...
    This is a regression test, not a hypothetical: before _lock_user existed
    this raised MySQL 1213 the first time it ran.
    """
    import threading
    ...
    try:
        ...
        with get_cursor(dictionary=False) as cursor:
            cursor.execute("""UPDATE password_reset_tokens SET expires_at = %s
                           ...
                           (datetime.now() - timedelta(minutes=1), user_id))
